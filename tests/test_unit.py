...
def test_update_cart(client):
    ...
    response = client.put(f'/api/cart/{email}', json={
        "items": [{"furniture_id": 10, "quantity": 3}]
    })
    assert response.status_code == 200
    data = response.get_json()
    ...
    response = client.put(f'/api/cart/{email}', json={
        "items": [{"furniture_id": 10, "quantity": 5}, {"furniture_id": 20, "quantity": 1}]
    })
    assert response.status_code == 200
    data = response.get_json()
    assert data["items"] == [{"furniture_id": 10, "quantity": 5}, {"furniture_id": 20, "quantity": 1}]
...
def test_update_inventory(client):
    ...
    response = client.put('/api/inventory/999', json={
        "price": 120.0,
